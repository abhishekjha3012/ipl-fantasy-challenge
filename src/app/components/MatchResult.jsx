...
export function MatchResult() {
  ...
  // Get all unique player IDs from match data
  const allPlayerIds = [...new Set(rawMatchData.flatMap(match => Object.keys(match.result || {})))];
    ...
    <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 shadow-2xl border border-white/20 relative overflow-hidden">
        <div className="flex items-center justify-between mb-4">
            ...
            <select
                ...
                className="bg-white/20 border border-white/30 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400"
            >
                <option value="">All Players</option>
                {allPlayerIds.map(playerId => (
                    <option key={playerId} value={playerId}>
                    {playerIdToName[playerId] || playerId}
                    </option>
                ))}
            </select>
        </div>
            ...
            <div className="overflow-x-auto" style={{ maxHeight: '50vh', overflowY: 'auto' }}>
                ...
                <table className="w-full text-white">
                    ...
                    {rawMatchData.map((match, index) => {
                        const result = match.result?.[selectedPlayer];
                        if (result === undefined) {
                        return (
                            <tr key={match.number || index} className="border-b border-white/10">
                            <td className="py-3 px-2 font-semibold">{match.number || (index + 1)}</td>
                            <td className="py-3 px-2">{match.match || 'Unknown Match'}</td>
                        ...
                        return (
                            <tr key={match.number || index} className="border-b border-white/10 hover:bg-white/5">
                                <td className="py-3 px-2 font-semibold">{match.number || (index + 1)}</td>
                                <td className="py-3 px-2">{match.match || 'Unknown Match'}</td>
                                <td className="py-3 px-2 text-center">{result}</td>
                            </tr>
                        );
                    })}
                    </tbody>
                ...
                // All players view (original table)
                <table className="w-full text-white">
                    <thead>
                    <tr className="border-b border-white/20">
                        <th className="text-left py-2 px-2">Match #</th>
